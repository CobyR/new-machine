...
$desired = @(
    '# Generated by new-machine bootstrap. Machine-specific; not tracked.'
    '[user]'
    "`tname = $($config.identity.name)"
    "`temail = $($config.identity.email)"
) -join "`r`n"

$current = if (Test-Path $localGitConfig) { (Get-Content $localGitConfig -Raw).TrimEnd() } else { $null }
if ($current -eq $desired) {
    Write-NMSkip 'git identity (~/.gitconfig.local)'
} else {
    Invoke-NMAction -Description 'write git identity to ~/.gitconfig.local' -Action {
        Set-Content -Path $localGitConfig -Value $desired -Encoding utf8
    } | Out-Null
}
